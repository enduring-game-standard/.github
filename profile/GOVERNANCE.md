...
This page states how the standard actually changes, who decides, and what your options are if you disagree. It describes the present arrangement honestly rather than performing a process that does not exist.

## The Present Arrangement: A Cathedral, On Purpose
...
- should not *claim to be* the Enduring Game Standard — call it something else so implementers know which contract they are reading. That is the only ask, and it is a request about honesty, not a restriction on use.
...
This standard currently has one author. That is a real risk and no messaging can dissolve it. The mitigations are structural rather than rhetorical: every specification is plain text under an MIT license in public repositories; the design rationale is recorded (in the specs, and at book length in *Enduring Games*); and nothing about the arrangement grants the author ongoing control — the day the specs are worth continuing without him, anyone can.
